...
export default function States() {
  ...
  useEffect(() => {
    if (Array.isArray(statesArray)) {
      setTotalStates(statesArray.length);
      setHasMore(statesArray.length === statesPerPage);
    }
  }, [statesArray, statesPerPage]);
  ...
  const handleNextPage = () => {
    if (hasMore && !statesLoading) {
      setCurrentPage(prev => prev + 1);
    }
  };

  const handlePrevPage = () => {
    if (currentPage > 0 && !statesLoading) {
      setCurrentPage(prev => prev - 1);
    }
  };
                  ...
                  <div className="relative">
                    <select
                      ...
                      onChange={(e) => {
                        ...
                        setCurrentPage(0); // Reset to first page when changing country filter
                      }}
            ...
            {statesLoading ? (
              ...
              <Table className="w-full min-w-[700px]">
                ...
                <TableBody className="bg-white divide-y divide-gray-200">
                  {filteredStates
                    .slice(currentPage * statesPerPage, (currentPage + 1) * statesPerPage)
                    .map((state: State) => {
                      return (
            ...
            <div className="px-6 py-3 border-t border-gray-200 bg-gray-50">
              <div className="flex items-center justify-between">
                <div className="text-sm text-gray-700">
                  Showing {totalStates} states on page {currentPage + 1}
                  {selectedCountry && ` (filtered by country)`}
                </div>
                <div className="flex items-center gap-2">
                  <button 
                    onClick={handlePrevPage}
                    disabled={currentPage === 0 || statesLoading}
                    className="p-2 text-gray-400 hover:text-gray-600 disabled:opacity-50"
                  >
                  ...
                  <button 
                    ...
                    disabled={!hasMore || statesLoading}
                    className="p-2 text-gray-400 hover:text-gray-600 disabled:opacity-50"
                  >
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
